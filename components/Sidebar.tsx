  ...
  return (
    <>
      <aside className="hidden md:flex w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex-col h-screen fixed top-0 left-0 z-20 transition-all duration-500">
        ...
        {/* Theme Toggle Slider – replaces static Link Status block */}
        <div className="p-6">
          <div className="p-5 bg-gradient-to-br from-blue-600/10 to-transparent border border-blue-600/20 rounded-3xl shadow-2xl shadow-blue-500/5">
            <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-3">Theme</p>
            <button
              onClick={toggleTheme}
              className="relative w-full h-10 bg-slate-200 dark:bg-slate-800 rounded-full p-1 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              {/* Background icons (light/dark) */}
              ...
              >
                {theme === 'light' ? (
                  <Icons.Sun size={14} className="text-amber-500" strokeWidth={2.5} />
                ) : (
                  <Icons.Moon size={14} className="text-indigo-300" strokeWidth={2.5} />
                )}
              </div>
            </button>
